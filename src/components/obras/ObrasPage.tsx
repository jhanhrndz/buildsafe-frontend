...
const ObrasPage = () => {
  ...
  const { obras, isLoading, error } = useObra();
  ...
  const filteredObras = useMemo(() => {
    ...
    return obras.filter(obra => 
      obra.nombre.toLowerCase().includes(searchTermLower) ||
      (obra.descripcion && obra.descripcion.toLowerCase().includes(searchTermLower))
    );
  }, [obras, searchTerm]);
  ...
  // Renderizar mensaje si no hay obras
  const renderEmptyState = () => {
    if (searchTerm) {
      return (
        <div className="bg-white p-8 rounded-lg shadow-sm text-center">
          <p className="text-gray-600 mb-2">No se encontraron obras que coincidan con "{searchTerm}"</p>
          <button
  ...
  return (
    <div className="space-y-6">
      {/* Cabecera con buscador y botón de creación */}
      <ObrasHeader
